...
def plot_2d_contour(
    surf_file, surf_name="train_loss", vmin=0.1, vmax=10, vlevel=0.5, show=False
):
    ...
    fig.savefig(
        surf_file + "_" + surf_name + "_2dcontour" + ".pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    fig.savefig(
        surf_file + "_" + surf_name + "_2dcontourf" + ".pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    sns_plot.get_figure().savefig(
        surf_file + "_" + surf_name + "_2dheat.pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    fig.savefig(
        surf_file + "_" + surf_name + "_3dsurface.pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
...
def plot_contour_trajectory(
    ...
    fig.savefig(
        proj_file + "_" + surf_name + "_2dcontour_proj.pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
...
def plot_2d_eig_ratio(surf_file, val_1="min_eig", val_2="max_eig", show=False):
    ...
    sns_plot.invert_yaxis()
    sns_plot.get_figure().savefig(
        surf_file + "_" + val_1 + "_" + val_2 + "_abs_ratio_heat_sns.pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    sns_plot.invert_yaxis()
    sns_plot.get_figure().savefig(
        surf_file + "_" + val_1 + "_" + val_2 + "_ratio_heat_sns.pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--surf_file", "-f", default="", help="The h5 file that contains surface values"
    )
    parser.add_argument(
        "--dir_file", default="", help="The h5 file that contains directions"
    )
    parser.add_argument(
        "--proj_file",
        default="",
        help="The h5 file that contains the projected trajectories",
    )
    parser.add_argument(
        "--surf_name", default="train_loss", help="The type of surface to plot"
    )
    ...
    parser.add_argument(
        "--zlim", default=10, type=float, help="Maximum loss value to show"
    )
    ...
    if exists(args.surf_file) and exists(args.proj_file) and exists(args.dir_file):
        ...
        plot_2d_contour(
            args.surf_file, args.surf_name, args.vmin, args.vmax, args.vlevel, args.show
        )
